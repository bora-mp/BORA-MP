...
namespace bora\plugin;

use bora\command\PluginCommand;
use bora\command\SimpleCommandMap;
use bora\event\Event;
use bora\event\EventPriority;
use bora\event\HandlerList;
use bora\event\Listener;
use bora\event\plugin\PluginDisableEvent;
use bora\event\plugin\PluginEnableEvent;
use bora\network\mcpe\protocol\ProtocolInfo;
use bora\permission\Permissible;
use bora\permission\Permission;
...
class PluginManager{
		...
		foreach($loaders ?? $this->fileAssociations as $loader){
			if($loader->canLoadPlugin($path)){
				...
				if($description instanceof PluginDescription){
					$this->server->getLogger()->info($this->server->getLanguage()->translateString("bora.plugin.load", [$description->getFullName()]));
					try{
	...
	public function loadPlugins(string $directory, array $newLoaders = null){
		...
		foreach($loaders as $loader){
			foreach(new \DirectoryIterator($directory) as $file){
				...
				try{
					...
					if(stripos($name, "bora") !== false or stripos($name, "minecraft") !== false or stripos($name, "mojang") !== false){
						$this->server->getLogger()->error($this->server->getLanguage()->translateString("bora.plugin.loadError", [$name, "%bora.plugin.restrictedName"]));
						continue;
					}elseif(strpos($name, " ") !== false){
						$this->server->getLogger()->warning($this->server->getLanguage()->translateString("bora.plugin.spacesDiscouraged", [$name]));
					}

					if(isset($plugins[$name]) or $this->getPlugin($name) instanceof Plugin){
						$this->server->getLogger()->error($this->server->getLanguage()->translateString("bora.plugin.duplicateError", [$name]));
						continue;
					}

					if(!$this->isCompatibleApi(...$description->getCompatibleApis())){
						$this->server->getLogger()->error($this->server->getLanguage()->translateString("bora.plugin.loadError", [
							$name,
							$this->server->getLanguage()->translateString("%bora.plugin.incompatibleAPI", [implode(", ", $description->getCompatibleApis())])
						]));
						continue;
					}
					...
					if(count($pluginMcpeProtocols = $description->getCompatibleMcpeProtocols()) > 0){
						...
						if(count(array_intersect($pluginMcpeProtocols, $serverMcpeProtocols)) === 0){
							$this->server->getLogger()->error($this->server->getLanguage()->translateString("bora.plugin.loadError", [
								$name,
								$this->server->getLanguage()->translateString("%bora.plugin.incompatibleProtocol", [implode(", ", $pluginMcpeProtocols)])
							]));
							continue;
						}
					}
					...
					$this->server->getLogger()->error($this->server->getLanguage()->translateString("bora.plugin.fileError", [$file, $directory, $e->getMessage()]));
					$this->server->getLogger()->logException($e);
				}
			}
		}
		...
		while(count($plugins) > 0){
			...
			foreach($plugins as $name => $file){
				if(isset($dependencies[$name])){
					foreach($dependencies[$name] as $key => $dependency){
						if(isset($loadedPlugins[$dependency]) or $this->getPlugin($dependency) instanceof Plugin){
							...
							$this->server->getLogger()->critical($this->server->getLanguage()->translateString("bora.plugin.loadError", [
								$name,
								$this->server->getLanguage()->translateString("%bora.plugin.unknownDependency", [$dependency])
							]));
							unset($plugins[$name]);
							continue 2;
						}
				...
				if(!isset($dependencies[$name]) and !isset($softDependencies[$name])){
					...
					if($plugin = $this->loadPlugin($file, $loaders) and $plugin instanceof Plugin){
						...
						$this->server->getLogger()->critical($this->server->getLanguage()->translateString("bora.plugin.genericLoadError", [$name]));
					}
				}
			}
			...
			if($missingDependency){
				...
				if($missingDependency){
					foreach($plugins as $name => $file){
						$this->server->getLogger()->critical($this->server->getLanguage()->translateString("bora.plugin.loadError", [$name, "%bora.plugin.circularDependency"]));
					}
					$plugins = [];
				}
			}
	...
	public function enablePlugin(Plugin $plugin){
		if(!$plugin->isEnabled()){
			try{
				$this->server->getLogger()->info($this->server->getLanguage()->translateString("bora.plugin.enable", [$plugin->getDescription()->getFullName()]));
	...
	protected function parseYamlCommands(Plugin $plugin) : array{
		...
		foreach($plugin->getDescription()->getCommands() as $key => $data){
			if(strpos($key, ":") !== false){
				$this->server->getLogger()->critical($this->server->getLanguage()->translateString("bora.plugin.commandError", [$key, $plugin->getDescription()->getFullName()]));
				continue;
			}
			if(is_array($data)){
				...
				if(isset($data["aliases"]) and is_array($data["aliases"])){
					...
					foreach($data["aliases"] as $alias){
						if(strpos($alias, ":") !== false){
							$this->server->getLogger()->critical($this->server->getLanguage()->translateString("bora.plugin.aliasError", [$alias, $plugin->getDescription()->getFullName()]));
							continue;
						}
						$aliasList[] = $alias;
					}
	...
	public function disablePlugin(Plugin $plugin){
		if($plugin->isEnabled()){
			$this->server->getLogger()->info($this->server->getLanguage()->translateString("bora.plugin.disable", [$plugin->getDescription()->getFullName()]));
	...
	public function registerEvent(string $event, Listener $listener, int $priority, EventExecutor $executor, Plugin $plugin, bool $ignoreCancelled = false) : void{
		...
		if(isset($tags["deprecated"]) and $this->server->getProperty("settings.deprecated-verbose", true)){
			$this->server->getLogger()->warning($this->server->getLanguage()->translateString("bora.plugin.deprecatedEvent", [
				$plugin->getName(),
				$event,
