...
class TimeCommand extends VanillaCommand{
	...
	public function __construct(string $name){
		parent::__construct(
			$name,
			"%bora.command.time.description",
			"%bora.command.time.usage"
		);
		$this->setPermission("bora.command.time.add;bora.command.time.set;bora.command.time.start;bora.command.time.stop");
	}
	...
	public function execute(CommandSender $sender, string $commandLabel, array $args){
		...
		if($args[0] === "start"){
			if(!$sender->hasPermission("bora.command.time.start")){
			...
			if(!$sender->hasPermission("bora.command.time.stop")){
				$sender->sendMessage(new TranslationContainer(TextFormat::RED . "%commands.generic.permission"));
			...
			if(!$sender->hasPermission("bora.command.time.query")){
				$sender->sendMessage(new TranslationContainer(TextFormat::RED . "%commands.generic.permission"));
		...
		if($args[0] === "set"){
			if(!$sender->hasPermission("bora.command.time.set")){
				$sender->sendMessage(new TranslationContainer(TextFormat::RED . "%commands.generic.permission"));
			...
			if(!$sender->hasPermission("bora.command.time.add")){
				$sender->sendMessage(new TranslationContainer(TextFormat::RED . "%commands.generic.permission"));
