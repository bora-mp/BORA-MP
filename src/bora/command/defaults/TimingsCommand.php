...
use bora\Player;
use bora\scheduler\BulkCurlTask;
use bora\Server;
...
class TimingsCommand extends VanillaCommand{
	...
	public function __construct(string $name){
		parent::__construct(
			$name,
			"%bora.command.timings.description",
			"%bora.command.timings.usage"
		);
		$this->setPermission("bora.command.timings");
	}
	...
	public function execute(CommandSender $sender, string $commandLabel, array $args){
		...
		if($mode === "on"){
			...
			$sender->sendMessage(new TranslationContainer("bora.command.timings.enable"));

			return true;
		}elseif($mode === "off"){
			TimingsHandler::setEnabled(false);
			$sender->sendMessage(new TranslationContainer("bora.command.timings.disable"));
			return true;
		}

		if(!TimingsHandler::isEnabled()){
			$sender->sendMessage(new TranslationContainer("bora.command.timings.timingsDisabled"));
		...
		if($mode === "reset"){
			...
			$sender->sendMessage(new TranslationContainer("bora.command.timings.reset"));
				...
				$sender->getServer()->getAsyncPool()->submitTask(new class($sender, $host, $agent, $data) extends BulkCurlTask{
					...
					public function onCompletion(Server $server){
						...
						if(isset($result[0]) && is_array($response = json_decode($result[0], true)) && isset($response["id"])){
							$sender->sendMessage(new TranslationContainer("bora.command.timings.timingsRead",
								["https://" . $this->host . "/?id=" . $response["id"]]));
						}else{
							$sender->sendMessage(new TranslationContainer("bora.command.timings.pasteError"));
						}
					}
				});
			}else{
				fclose($fileTimings);
				$sender->sendMessage(new TranslationContainer("bora.command.timings.timingsWrite", [$timings]));
			}
		}
