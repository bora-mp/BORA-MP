...
use bora\command\CommandReader;
use bora\command\CommandSender;
use bora\command\ConsoleCommandSender;
use bora\command\PluginIdentifiableCommand;
use bora\command\SimpleCommandMap;
use bora\entity\Entity;
use bora\entity\Skin;
use bora\event\HandlerList;
use bora\event\level\LevelInitEvent;
use bora\event\level\LevelLoadEvent;
use bora\event\player\PlayerDataSaveEvent;
use bora\event\server\CommandEvent;
use bora\event\server\QueryRegenerateEvent;
use bora\event\server\ServerCommandEvent;
use bora\inventory\CraftingManager;
use bora\item\enchantment\Enchantment;
use bora\item\Item;
use bora\item\ItemFactory;
use bora\lang\BaseLang;
use bora\lang\TextContainer;
use bora\lang\TranslationContainer;
use bora\level\biome\Biome;
use bora\level\format\io\LevelProvider;
use bora\level\format\io\LevelProviderManager;
use bora\level\generator\Generator;
use bora\level\generator\GeneratorManager;
use bora\level\Level;
use bora\level\LevelException;
use bora\metadata\EntityMetadataStore;
use bora\metadata\LevelMetadataStore;
use bora\metadata\PlayerMetadataStore;
...
use bora\network\rcon\RCON;
use bora\network\upnp\UPnP;
use bora\permission\BanList;
use bora\permission\DefaultPermissions;
use bora\permission\PermissionManager;
use bora\plugin\PharPluginLoader;
use bora\plugin\FolderPluginLoader;
use bora\plugin\Plugin;
use bora\plugin\PluginLoadOrder;
use bora\plugin\PluginManager;
use bora\plugin\ScriptPluginLoader;
use bora\resourcepacks\ResourcePackManager;
use bora\scheduler\AsyncPool;
use bora\scheduler\SendUsageTask;
use bora\snooze\SleeperHandler;
use bora\snooze\SleeperNotifier;
use bora\tile\Tile;
use bora\timings\Timings;
use bora\timings\TimingsHandler;
use bora\updater\AutoUpdater;
 ...
 */
class Server{
	public const BROADCAST_CHANNEL_ADMINISTRATIVE = "bora.broadcast.admin";
	public const BROADCAST_CHANNEL_USERS = "bora.broadcast.user";
	 ...
	 * @return string
	 */
	public function getBoraVersion() : string{
		return \bora\VERSION;
	}
	...
	public function getMotd() : string{
		return $this->getConfigString("motd", \bora\NAME . " Sunucu");
	}
		...
		if($this->shouldSavePlayerData()){
			if(file_exists($path . "$name.dat")){
				try{
					...
					$this->logger->notice($this->getLanguage()->translateString("bora.data.playerCorrupted", [$name]));
				}
			}else{
				$this->logger->notice($this->getLanguage()->translateString("bora.data.playerNotFound", [$name]));
			}
		}
	...
	public function saveOfflinePlayerData(string $name, CompoundTag $nbtTag){
		...
		if(!$ev->isCancelled()){
			...
			try{
				...
				$this->logger->critical($this->getLanguage()->translateString("bora.data.saveError", [$name, $e->getMessage()]));
	...
	public function loadLevel(string $name) : bool{
		...
		if($this->isLevelLoaded($name)){
			...
			$this->logger->notice($this->getLanguage()->translateString("bora.level.notFound", [$name]));
		...
		if($providerClass === null){
			$this->logger->error($this->getLanguage()->translateString("bora.level.loadError", [$name, "Cannot identify format of world"]));
	 ...
	 * @param string      $name
	 * @param int|null    $seed
	 * @param string|null $generator Class name that extends bora\level\generator\Generator
	 * @param array       $options
	 *
	 * @return bool
	 */
	public function generateLevel(string $name, int $seed = null, $generator = null, array $options = []) : bool{
		...
		$this->getLogger()->notice($this->getLanguage()->translateString("bora.level.backgroundGeneration", [$name]));
	...
	public function __construct(\ClassLoader $autoloader, \AttachableThreadedLogger $logger, string $dataPath, string $pluginPath){
		...
		try{
			...
			if(!file_exists($this->dataPath . "bora.yml")){
				$content = file_get_contents(\bora\RESOURCE_PATH . "bora.yml");
				if(\bora\IS_DEVELOPMENT_BUILD){
					$content = str_replace("preferred-channel: stable", "preferred-channel: beta", $content);
				}
				@file_put_contents($this->dataPath . "bora.yml", $content);
			}
			$this->config = new Config($this->dataPath . "bora.yml", Config::YAML, []);
			$this->properties = new Config($this->dataPath . "server.properties", Config::PROPERTIES, [
				"motd" => \bora\NAME . " Server",
				"server-port" => 19132,
				"white-list" => false,
			...
			define('bora\DEBUG', (int) $this->getProperty("debug.level", 1));
            $yazi = "
            \n
§b┌────────────────────────────────────────────────────────┐  §b-- Yükleme: Özellikler ve Yapılandırma --
§b│{}                                    	                 §b│  §6Sunucu: §f" . $this->getName() . "
§b│{}                                                      §b│  §6API: §f " . $this->getApiVersion() . "
§b│{}		    				         §b│  §6Bedrock Sürüm: §f" . $this->getVersion() . "
§b│{}    ____   ____  _____                 __  __ _____   §b│  §6Protokol Sürüm: §f" . ProtocolInfo::CURRENT_PROTOCOL . "
§b│{}   |  _ \ / __ \|  __ \     /\        |  \/  |  __ \  §b│
§b│{}   | |_) | |  | | |__) |   /  \ ______| \  / | |__) | §b│  §b------------------------------------------
§b│{}   |  _ <| |  | |  _  /   / /\ \______| |\/| |  ___/  §b│
§b│{}   | |_) | |__| | | \ \  / ____ \     | |  | | |      §b│  §6Dil: §f" . $this->properties->get("language") . "
§b│{}   |____/ \____/|_|  \_\/_/    \_\    |_|  |_|_|      §b│  §6Github: §fgithub.com/bora-mp/BORA-MP
§b│{}                                                      §b│  §6Discord: §fdiscord.me/bora
§b│{}                                                      §b│  §6Website: §fbora.kodmadeni.com
§b│{}						         §b│  §6Forum: §fbora.kodmadeni.com/forum/
§b│{}		    				         §b│
§b└────────────────────────────────────────────────────────┘
";
			$this->logger->info($yazi);
            $this->logger->info("Yükleniyor bora.yml...");
            $this->logger->info("Yükleniyor server properties...");
            ...
            if(\bora\IS_DEVELOPMENT_BUILD and !((bool) $this->getProperty("settings.enable-dev-builds", false))){
				$this->logger->emergency($this->baseLang->translateString("bora.server.devBuild.error1", [\bora\NAME]));
				$this->logger->emergency($this->baseLang->translateString("bora.server.devBuild.error4", ["enable-dev-builds"]));
				$this->forceShutdown();
				return;
			}
			...
			if($this->logger instanceof MainLogger){
				$this->logger->setLogDebug(\bora\DEBUG > 1);
			}

			$this->memoryManager = new MemoryManager($this);

			$this->logger->info($this->getLanguage()->translateString("bora.server.start", [TextFormat::AQUA . $this->getVersion() . TextFormat::RESET]));
            $this->logger->info($this->getLanguage()->translateString("bora.server.start.boraapi", [TextFormat::AQUA . $this->getApiVersion() . TextFormat::RESET]));
			...
			if($this->onlineMode){
				$this->logger->notice($this->getLanguage()->translateString("bora.server.auth.enabled"));
				$this->logger->notice($this->getLanguage()->translateString("bora.server.authProperty.enabled"));
			}else{
				$this->logger->warning($this->getLanguage()->translateString("bora.server.auth.disabled"));
				$this->logger->warning($this->getLanguage()->translateString("bora.server.authWarning"));
				$this->logger->warning($this->getLanguage()->translateString("bora.server.authProperty.disabled"));
			}
			...
			if(\bora\DEBUG >= 0){
				@cli_set_process_title($this->getName() . " " . $this->getBoraVersion());
			}

			$this->logger->info($this->getLanguage()->translateString("bora.server.networkStart", [$this->getIp(), $this->getPort()]));
			...
			$this->logger->info($this->getLanguage()->translateString("bora.server.license", [$this->getName()]));
			...
			if(extension_loaded("leveldb")){
				$this->logger->debug($this->getLanguage()->translateString("bora.debug.enable"));
			}
			...
			if($this->getDefaultLevel() === null){
				$default = $this->getConfigString("level-name", "dunya");
				if(trim($default) == ""){
					$this->getLogger()->warning("level-name cannot be null, using default");
					$default = "dunya";
					$this->setConfigString("level-name", "dunya");
				}
			...
			if(!($this->getDefaultLevel() instanceof Level)){
				$this->getLogger()->emergency($this->getLanguage()->translateString("bora.level.defaultError"));
				$this->forceShutdown();
	...
	private function start(){
		...
		$this->logger->info($this->getLanguage()->translateString("bora.server.defaultGameMode", [self::getGamemodeString($this->getGamemode())]));

		$this->logger->info($this->getLanguage()->translateString("bora.server.startFinished", [round(microtime(true) - \bora\START_TIME, 3)]));
	...
	public function crashDump(){
		...
		try{
			$this->logger->emergency($this->getLanguage()->translateString("bora.crash.create"));
			$dump = new CrashDump($this);

			$this->logger->emergency($this->getLanguage()->translateString("bora.crash.submit", [$dump->getPath()]));
			...
			if($this->getProperty("auto-report.enabled", true) !== false){
				...
				if(strrpos(\bora\GIT_COMMIT, "-dirty") !== false or \bora\GIT_COMMIT === str_repeat("00", 20)){
				...
				if($report){
					...
					$reply = Internet::postURL($url, [
						"report" => "yes",
						"name" => $this->getName() . " " . $this->getBoraVersion(),
						"email" => "crash@bora.net",
					...
					if($reply !== false and ($data = json_decode($reply)) !== null and isset($data->crashId) and isset($data->crashUrl)){
						...
						$this->logger->emergency($this->getLanguage()->translateString("bora.crash.archive", [$reportUrl, $reportId]));
					}
				}
			}
		}catch(\Throwable $e){
			$this->logger->logException($e);
			try{
				$this->logger->critical($this->getLanguage()->translateString("bora.crash.error", [$e->getMessage()]));
			}catch(\Throwable $e){}
		}
		...
		$spacing = ((int) \bora\START_TIME) - time() + 120;
		if($spacing > 0){
			echo "--- Bekleyin. $spacing saniye sonra sunucu yeniden başlatılacak (Kendiniz de yeniden başlatabilirsiniz) ---" . PHP_EOL;
			sleep($spacing);
		}
		@Utils::kill(getmypid());
		exit(1);
	...
	private function titleTick(){
		...
		echo "\x1b]0;" . $this->getName() . " " .
			$this->getBoraVersion() .
			" | Aktif " . count($this->players) . "/" . $this->getMaxPlayers() .
			" | Bellek " . $usage .
	...
	private function tick() : void{
		...
		if(($this->tickCounter % 100) === 0){
			...
			if($this->getTicksPerSecondAverage() < 12){
				$this->logger->warning($this->getLanguage()->translateString("bora.server.tickOverload"));
			}
		}
