...
class VerifyLoginTask extends AsyncTask{
	...
	private function validateToken(string $jwt, ?string &$currentPublicKey, bool $first = false) : void{
		...
		if($currentPublicKey === null){
			if(!$first){
				throw new VerifyLoginException("%bora.disconnect.invalidSession.missingKey");
			}
		...
		if($v !== 1){
			throw new VerifyLoginException("%bora.disconnect.invalidSession.badSignature");
		}
		...
		if(isset($claims["nbf"]) and $claims["nbf"] > $time + self::CLOCK_DRIFT_MAX){
			throw new VerifyLoginException("%bora.disconnect.invalidSession.tooEarly");
		}

		if(isset($claims["exp"]) and $claims["exp"] < $time - self::CLOCK_DRIFT_MAX){
			throw new VerifyLoginException("%bora.disconnect.invalidSession.tooLate");
		}
