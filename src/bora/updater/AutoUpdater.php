...
use bora\event\server\UpdateNotifyEvent;
use bora\Player;
use bora\Server;
...
class AutoUpdater{
	...
	public function checkUpdateCallback(array $updateInfo){
		...
		if($this->hasUpdate()){
			...
			if(!\bora\IS_DEVELOPMENT_BUILD and $this->getChannel() !== "stable"){
				$this->showChannelSuggestionStable();
			}elseif(\bora\IS_DEVELOPMENT_BUILD and $this->getChannel() === "stable"){
	...
	public function showConsoleUpdate(){
		$messages = [
			"Sunucu sürümü " . $this->server->getBoraVersion() . " süresi doldu."
		];
		if($this->updateInfo["details_url"] !== null){
			$messages[] = "Details: " . $this->updateInfo["details_url"];
		}
		$messages[] = "Download: /update.sh";
	...
	protected function showChannelSuggestionStable(){
		$this->printConsoleMessage([
			...
			"If you would like to be kept informed about new Stable builds only, it is recommended that you change 'preferred-channel' in your bora.yml to 'stable'."
		]);
	}
	...
	protected function showChannelSuggestionBeta(){
		$this->printConsoleMessage([
			...
			"If you would like to be kept informed about new Beta or Development builds, it is recommended that you change 'preferred-channel' in your bora.yml to 'beta' or 'development'."
		]);
	}
	...
	protected function printConsoleMessage(array $lines, string $logLevel = \LogLevel::INFO){
		...
		$title = $this->server->getName() . ' Oto Güncelleyici';
	...
	protected function checkUpdate(){
		...
		$currentVersion = new VersionString(\bora\BASE_VERSION, \bora\IS_DEVELOPMENT_BUILD, \bora\BUILD_NUMBER);
		try{
