...
use bora\entity\projectile\Arrow;
use bora\entity\Skin;
use bora\event\entity\EntityDamageByEntityEvent;
use bora\event\entity\EntityDamageEvent;
use bora\event\inventory\InventoryCloseEvent;
use bora\event\player\cheat\PlayerIllegalMoveEvent;
use bora\event\player\PlayerAchievementAwardedEvent;
use bora\event\player\PlayerAnimationEvent;
use bora\event\player\PlayerBedEnterEvent;
use bora\event\player\PlayerBedLeaveEvent;
use bora\event\player\PlayerBlockPickEvent;
use bora\event\player\PlayerChangeSkinEvent;
use bora\event\player\PlayerChatEvent;
use bora\event\player\PlayerCommandPreprocessEvent;
use bora\event\player\PlayerDeathEvent;
use bora\event\player\PlayerEditBookEvent;
use bora\event\player\PlayerExhaustEvent;
use bora\event\player\PlayerGameModeChangeEvent;
use bora\event\player\PlayerInteractEvent;
use bora\event\player\PlayerItemConsumeEvent;
use bora\event\player\PlayerJoinEvent;
use bora\event\player\PlayerJumpEvent;
use bora\event\player\PlayerKickEvent;
use bora\event\player\PlayerLoginEvent;
use bora\event\player\PlayerMoveEvent;
use bora\event\player\PlayerPreLoginEvent;
use bora\event\player\PlayerQuitEvent;
use bora\event\player\PlayerRespawnEvent;
use bora\event\player\PlayerToggleFlightEvent;
use bora\event\player\PlayerToggleSneakEvent;
use bora\event\player\PlayerToggleSprintEvent;
use bora\event\player\PlayerTransferEvent;
use bora\event\server\DataPacketSendEvent;
use bora\form\Form;
use bora\form\FormValidationException;
use bora\inventory\CraftingGrid;
use bora\inventory\Inventory;
use bora\inventory\PlayerCursorInventory;
use bora\inventory\transaction\action\InventoryAction;
use bora\inventory\transaction\CraftingTransaction;
use bora\inventory\transaction\InventoryTransaction;
use bora\inventory\transaction\TransactionValidationException;
use bora\item\Consumable;
use bora\item\Durable;
use bora\item\enchantment\EnchantmentInstance;
use bora\item\enchantment\MeleeWeaponEnchantment;
use bora\item\Item;
use bora\item\WritableBook;
use bora\item\WrittenBook;
use bora\lang\TextContainer;
use bora\lang\TranslationContainer;
use bora\level\ChunkLoader;
use bora\level\format\Chunk;
use bora\level\Level;
use bora\level\Location;
use bora\level\Position;
use bora\math\AxisAlignedBB;
use bora\math\Vector3;
use bora\metadata\MetadataValue;
...
use bora\permission\PermissibleBase;
use bora\permission\PermissionAttachment;
use bora\permission\PermissionAttachmentInfo;
use bora\permission\PermissionManager;
use bora\plugin\Plugin;
use bora\resourcepacks\ResourcePack;
use bora\tile\ItemFrame;
use bora\tile\Spawnable;
use bora\tile\Tile;
...
class Player extends Human implements CommandSender, ChunkLoader, IPlayer{
	...
	protected function processMovement(int $tickDiff){
		...
		if(!$revert and $distanceSquared != 0){
			...
			if($this->isSurvival() and !$revert and $diff > 0.0625){
				...
				if(!$ev->isCancelled()){
					$revert = true;
					$this->server->getLogger()->warning($this->getServer()->getLanguage()->translateString("bora.player.invalidMove", [$this->getName()]));
					$this->server->getLogger()->debug("Old position: " . $this->asVector3() . ", new position: " . $this->newPosition);
				}
			}
	...
	public function handleLogin(LoginPacket $packet) : bool{
		...
		if($packet->protocol !== ProtocolInfo::CURRENT_PROTOCOL){
			...
			//This bora disconnect message will only be seen by the console (PlayStatusPacket causes the messages to be shown for the client)
			$this->close("", $this->server->getLanguage()->translateString("bora.disconnect.incompatibleProtocol", [$packet->protocol ?? "unknown"]), false);
	...
	public function onVerifyCompleted(LoginPacket $packet, ?string $error, bool $signedByMojang) : void{
		...
		if($error !== null){
			$this->close("", $this->server->getLanguage()->translateString("bora.disconnect.invalidSession", [$error]));
			return;
		}
	...
	protected function completeLoginSequence(){
		...
		$this->server->getLogger()->info($this->getServer()->getLanguage()->translateString("bora.player.logIn", [
				...
				switch($type){
					...
					case InventoryTransactionPacket::USE_ITEM_ON_ENTITY_ACTION_ATTACK:
						...
						if($target instanceof ItemEntity or $target instanceof Arrow){
							...
							$this->server->getLogger()->warning($this->getServer()->getLanguage()->translateString("bora.player.invalidEntity", [$this->getName()]));
							return false;
						}
	...
	public function sendTranslation(string $message, array $parameters = []){
		...
		if(!$this->server->isLanguageForced()){
			...
			$pk->message = $this->server->getLanguage()->translateString($message, $parameters, "bora.");
			foreach($parameters as $i => $p){
				$parameters[$i] = $this->server->getLanguage()->translateString($p, $parameters, "bora.");
			}
	...
	final public function close($message = "", string $reason = "generic reason", bool $notify = true) : void{
		if($this->isConnected() and !$this->closed){
			...
			$this->server->getLogger()->info($this->getServer()->getLanguage()->translateString("bora.player.logOut", [
