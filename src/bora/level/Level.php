 ...
 */
namespace bora\level;

use bora\block\Block;
use bora\block\BlockFactory;
use bora\entity\Entity;
use bora\entity\object\ExperienceOrb;
use bora\entity\object\ItemEntity;
use bora\event\block\BlockBreakEvent;
use bora\event\block\BlockPlaceEvent;
use bora\event\block\BlockUpdateEvent;
use bora\event\level\ChunkLoadEvent;
use bora\event\level\ChunkPopulateEvent;
use bora\event\level\ChunkUnloadEvent;
use bora\event\level\LevelSaveEvent;
use bora\event\level\LevelUnloadEvent;
use bora\event\level\SpawnChangeEvent;
use bora\event\player\PlayerInteractEvent;
use bora\item\Item;
use bora\item\ItemFactory;
use bora\level\biome\Biome;
use bora\level\format\Chunk;
use bora\level\format\ChunkException;
use bora\level\format\EmptySubChunk;
use bora\level\format\io\BaseLevelProvider;
use bora\level\format\io\ChunkRequestTask;
use bora\level\format\io\exception\CorruptedChunkException;
use bora\level\format\io\exception\UnsupportedChunkFormatException;
use bora\level\format\io\LevelProvider;
use bora\level\generator\Generator;
use bora\level\generator\GeneratorManager;
use bora\level\generator\GeneratorRegisterTask;
use bora\level\generator\GeneratorUnregisterTask;
use bora\level\generator\PopulationTask;
use bora\level\light\BlockLightUpdate;
use bora\level\light\LightPopulationTask;
use bora\level\light\SkyLightUpdate;
use bora\level\particle\DestroyBlockParticle;
use bora\level\particle\Particle;
use bora\level\sound\Sound;
use bora\math\AxisAlignedBB;
use bora\math\Vector2;
use bora\math\Vector3;
use bora\metadata\BlockMetadataStore;
use bora\metadata\Metadatable;
...
use bora\Server;
use bora\tile\Chest;
use bora\tile\Container;
use bora\tile\Tile;
use bora\timings\Timings;
use bora\utils\ReversePriorityQueue;
use function abs;
...
class Level implements ChunkManager, Metadatable{
	...
	public function __construct(Server $server, string $name, LevelProvider $provider){
		...
		$this->server->getLogger()->info($this->server->getLanguage()->translateString("bora.level.preparing", [$this->displayName]));
	...
	public function unload(bool $force = false) : bool{
		...
		$this->server->getLogger()->info($this->server->getLanguage()->translateString("bora.level.unloading", [$this->getName()]));
	...
	public function checkSpawnProtection(Player $player, Vector3 $vector) : bool{
		if(!$player->hasPermission("bora.spawnprotect.bypass") and ($distance = $this->server->getSpawnRadius()) > -1){
